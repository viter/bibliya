...
export function scroller(this: any) {
  this.rozdily.forEach((rozdil: Element) => {
    ...
    if (rect.top < 350 && rect.top > 50) {
      ...
      if (!this.activeItem.classList.contains('dark:text-neutral-100')) {
        this.prevItem.classList.add('dark:text-neutral-400');
        this.activeItem.classList.remove('dark:text-neutral-400');
        this.activeItem.classList.add('dark:text-neutral-100');
        if (this.prevItem) {
          this.prevItem.classList.remove('dark:text-neutral-100');
          this.prevItem.classList.add('dark:text-neutral-400');
        }
      }
